-- place_order, with the coupon rules it was missing.
--
-- Two changes: the discount is computed against the delivery fee as well, so a
-- free-delivery code works; and the redemption is written to the ledger inside
-- the same transaction as the order, which is what makes "one per customer"
-- true rather than advisory. `used_count` is no longer bumped by hand — a
-- trigger on the ledger owns it.
...
begin
  ...
  v_delivery_fee := v_vendor.delivery_fee;
  ...
  -- Inside the same transaction as the order it belongs to: if the insert
  -- below fails on its unique constraint, the whole order rolls back rather
  -- than a second redemption slipping through.
