-- place_order now refuses addresses outside the delivery footprint.
--
-- The address picker already warns when the pin falls outside coverage, but
-- that is a hint, not a control: the RPC is the only place a customer cannot
-- route around. is_within_service_area() allows everything while no areas are
-- configured, so this is inert until the admin draws the first circle.
